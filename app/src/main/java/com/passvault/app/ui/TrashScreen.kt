...
fun TrashScreen(onBack: () -> Unit) {
    ...
    val trashed = VaultRepository.entries.filter { it.isDeleted }
                    ...
                    Card(Modifier.fillMaxWidth()) {
                        Row(
                            ...
                            Column(Modifier.weight(1f)) {
                                ...
                                val days = (System.currentTimeMillis() - entry.deletedAt) / 86_400_000L
                                Text(
                                    stringResource(R.string.trash_deleted_ago, days),
                                    style = MaterialTheme.typography.bodySmall,
                                    color = MaterialTheme.colorScheme.onSurfaceVariant,
                                )
                            }
                            IconButton(onClick = { VaultRepository.restoreEntry(context, entry.id) }) {
                                Icon(
                            ...
                            IconButton(onClick = { VaultRepository.deleteForever(context, entry.id) }) {
                                Icon(
    ...
    if (confirmEmpty) {
        AlertDialog(
            ...
            confirmButton = {
                TextButton(onClick = {
                    confirmEmpty = false
                    VaultRepository.emptyTrash(context)
